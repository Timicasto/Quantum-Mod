...
public class WorldGenPetroleumLake extends WorldGenerator {
    private Block block;
    public WorldGenPetroleumLake() {
        this.block = QuantumBaseBlocks.petroleum;
    }
        ...
        for (int i = 0 ; i < 16 ; ++i) {
            for (int j2 = 0 ; j2 < 16 ; ++j2) {
                for (int j1 = 0 ; j1 < 8 ; ++j1) {
                    flag = !buffer[(i * 16 + j2) * 8 + j1] && buffer[((i + 1) * 16 + j2) * 8 + j1] || i > 0 && buffer[((i - 1 ) * 16 + j2) + 8 + j1] || j2 < 15 && buffer[(i * 16 + j2 + 1) * 8 + j1] || j2 > 0 && buffer[(i * 16 + (j2 - 1)) * 8 + j1] || j1 < 7 && buffer[(i * 16 + j2) * 8 + j1 + 1] || j1 > 0 && buffer[(i * 16 + j2) * 8 + (j1 - 1)];
                    if (flag) {
